...
const findAddresses = async (userId) => {
  const [rows] = await pool.execute(
    `SELECT id, label, address_type, first_name, last_name, complement, street, street_number, city, zip, country, canton, phone, is_default
     FROM addresses
     WHERE user_id = ?
...
const createAddress = async (userId, { label, addressType, firstName, lastName, complement, street, streetNumber, city, zip, country, canton, phone, isDefault }) => {
  ...
  const [result] = await pool.execute(
    `INSERT INTO addresses (user_id, label, address_type, first_name, last_name, complement, street, street_number, city, zip, country, canton, phone, is_default)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [userId, label, addressType || 'both', firstName || null, lastName || null, complement || null, street, streetNumber || null, city, zip, country || 'CH', canton || null, phone || null, isDefault ? 1 : 0]
  );
  return result.insertId;
};

// Mise à jour d'une adresse
// `isDefault` null = statut « par défaut » inchangé
const updateAddress = async (addressId, userId, { label, addressType, firstName, lastName, complement, street, streetNumber, city, zip, country, canton, phone, isDefault }) => {
  if (isDefault) {
    await pool.execute(
      `UPDATE addresses SET is_default = 0 WHERE user_id = ?`,
      [userId]
  ...
  await pool.execute(
    `UPDATE addresses SET label = ?, address_type = ?, first_name = ?, last_name = ?, complement = ?, street = ?, street_number = ?, city = ?, zip = ?, country = ?, canton = ?, phone = ?, is_default = COALESCE(?, is_default)
     WHERE id = ? AND user_id = ?`,
    [label, addressType || 'both', firstName || null, lastName || null, complement || null, street, streetNumber || null, city, zip, country || 'CH', canton || null, phone || null, isDefault === null || isDefault === undefined ? null : (isDefault ? 1 : 0), addressId, userId]
  );
};
  ...
  try {
    ...
    await connection.execute(
      `UPDATE orders SET
         ...
         shipping_complement = NULL, shipping_street = NULL, shipping_street_number = NULL,
         shipping_city = NULL, shipping_zip = NULL, shipping_canton = NULL,
         shipping_phone = NULL,
         billing_first_name = 'Supprimé', billing_last_name = 'Supprimé',
         billing_complement = NULL, billing_street = NULL, billing_street_number = NULL,
