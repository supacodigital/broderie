...
CREATE TABLE users (
  ...
  -- Réinitialisation de mot de passe (jeton SHA-256 hex, envoyé par email)
...
CREATE TABLE products (
  ...
  UNIQUE KEY uq_products_external_ref (external_ref),   -- rejeu de l'import catalogue sans doublon
  -- Index simples
...
-- ------------------------------------------------------------
-- Les ~15 500 produits proviennent de l'export du logiciel métier de
-- la cliente (donnéesclient/V_ArticleC_INT.xlsx), chargés via
-- `npm run import:catalog` (voir docs/IMPORT-CATALOGUE.md).
--
-- Aucun produit / fournisseur de démo n'est seedé ici : leurs SKU
-- (RefFab) et slugs entraient en collision avec ceux de l'export
-- (uq_products_sku / uq_products_slug), ce qui faisait écraser des
-- fiches par l'import au lieu d'en créer. Le catalogue a donc une
-- source unique : l'import.
--
-- Fournisseurs : la table `suppliers` reste vide au départ. La cliente
-- crée ses fournisseurs depuis le back-office (CRUD livré) puis les
-- rattache aux produits (products.supplier_id) — cf. §7 du plan d'import.
--
-- AUTO_INCREMENT : on part des valeurs par défaut (1). Les id produits
-- importés sont attribués par MySQL ; aucune plage réservée.
-- ============================================================
